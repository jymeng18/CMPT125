...
int main( int argc, char * argv[] ) {
  ...
  intArray_sort( test_ia);

  printf("Printing test_ia\n"); 
  intArray_print( test_ia );

  // Testing Copy
  printf("Copying test_ia\n");
  intArray_print(intArray_copy( test_ia ));

  printf("Storing data structure into the json file %s\n", argv[2] );
  if(intArray_write_to_json( test_ia, argv[2]) != 0){
    puts("Did not work!");
  }
  
  // Testing load json function
  printf("testing load json function\n");
  intArray_t * test_ia2 = intArray_load_from_json(argv[2]);

  // printing test_ia2
  printf("testing test_ia2\n");
  intArray_print(test_ia2);
